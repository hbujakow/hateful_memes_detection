...
def main(args) -> None:
    ...
    test_memes = pd.read_json(os.path.join(data_path, "dev_seen.jsonl"), lines=True) # we use dev_seen for testing as in the original paper for comparison
    ...
    for category, question in tqdm(categories.items(), desc="Generating captions for categories", position=0):
        for data_split, dataset in tqdm(data.items(), desc=f"Generating captions [{category}]", position=1, leave=False):
            captions = {}
            for i, path in enumerate(dataset):
                if i % 250 == 0:
                    print(f"Already preprocessing {i*100/len(dataset):2f}% of the {data_split} dataset")
                image = Image.open(os.path.join(data_path, 'inpainted', path))
                caption = generate_prompt_result(model, vis_processors, device, image, question)
                captions[path] = caption
            pkl.dump(
                captions, open(os.path.join(args.output_path, f"{category}_{data_split}.pkl"), "wb")
            )
