...
def evaluate_predictions(data_path: str) -> Dict[str, Any]:
    ...
    model_name = match.group(1) #match.group(1) if match and match.group(2) else match.group(1)
    ...
    return {"model": [model_name], f"auc_{set_var}": [round(roc_auc_score(y, y_pred), 4)], f"accuracy_{set_var}": [round(accuracy_score(y, y_pred), 4)]}


def main(args):
    datasets_list = get_datasets(args.data_path)

    df_results = pd.DataFrame(columns=["model", "auc_train", "accuracy_train", "auc_dev", "accuracy_dev", "auc_test", "accuracy_test"])
    ...
    df_results['no_epochs'] = df_results['model'].apply(lambda x: re.search(r'\d+', x).group() if re.search(r'\d+', x) else '0').astype(int)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-path', type=str, default=DATAPATH, help='path to the data with LLaVA predictions')
