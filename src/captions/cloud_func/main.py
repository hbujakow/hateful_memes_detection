...
def generate_captions(im):
    ...
    response = ""
    for category in "generic,race,gender,country,animal,valid_disabled,religion".split(
        ","
    ):
...
async def generate_captions_endpoint(request: HttpRequest):
    try:
        image_data = request.json().get("image", "")
        im = process_image(image_data)
        result = generate_captions(im)
